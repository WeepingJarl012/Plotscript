...
void NotebookTest::testContinuousPlotLayout() {
    ...
    // check ordinate min label
    QCOMPARE(findText(scene, QPointF(xmin-2, -ymin), 0, QString("0")), 1);
    
    // check ordinate max label
    QCOMPARE(findText(scene, QPointF(xmin-2, -ymax), 0, QString("0")), 1);
...
void NotebookTest::testDiscretePlot() {
    ...
    // check the bounding box left and (-2, -3) stem
    QCOMPARE(findLines(scene, QRectF(xmin, -ymax, 0, 20), 0.1), 2);
    
    // check the bounding box right and (2, 5) stem
    QCOMPARE(findLines(scene, QRectF(xmax, -ymax, 0, 20), 0.1), 2);
    
    // check the abscissa axis
    QCOMPARE(findLines(scene, QRectF(xmin, 0 + (20/8), 20, 0), 0.6), 2);
    
    // check the ordinate axis and (0, 1) stem
    QCOMPARE(findLines(scene, QRectF(0, -ymax, 0, 20), 0.1), 2);
    
    // check the point at (-2,-3)
    QCOMPARE(findPoints(scene, QPointF(-10, 10), 0.6), 1);
    
    // check the point at (-1.5,-2)
    QCOMPARE(findPoints(scene, QPointF(-10 + (20/8), 10 - (20/8)), 0.8), 1);
    
    // check the point at (-1,-1)
    QCOMPARE(findPoints(scene, QPointF(-10 + (40/8), 10 - (40/8)), 0.8), 1);
    
    // check the point at (-0.5,0)
    QCOMPARE(findPoints(scene, QPointF(-10 + (60/8), 10 - (60/8)), 0.8), 1);
    
    // check the point at (0,1)
    QCOMPARE(findPoints(scene, QPointF(0, 0), 0.6), 1);
    
    // check the point at (0.5,2)
    QCOMPARE(findPoints(scene, QPointF(10 - (60/8), -10 + (60/8)), 0.8), 1);
    
    // check the point at (1,3)
    QCOMPARE(findPoints(scene, QPointF(10 - (40/8), -10 + (40/8)), 0.8), 1);
    
    // check the point at (1.5,4)
    QCOMPARE(findPoints(scene, QPointF(10 - (20/8), -10 + (20/8)), 0.8), 1);
    
    // check the point at (2,5)
    QCOMPARE(findPoints(scene, QPointF(10, -10), 0.6), 1);
}
